...
def main(args=None):
    ...
    sim = SIM(world_file="worlds/robot_world.yaml", disable_plotting=True)  # instantiate environment
    ...
    while epoch < max_epochs:  # train until max_epochs is reached
        ...
        if (
            ...
            episode += 1
            print(f"Epoch {epoch+1}/{max_epochs} | Episode {episode}/{episodes_per_epoch} | Steps {steps} | {'GOAL' if goal else 'COLLISION' if collision else 'timeout'}", flush=True)

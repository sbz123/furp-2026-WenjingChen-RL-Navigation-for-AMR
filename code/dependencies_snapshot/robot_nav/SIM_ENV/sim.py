...
class SIM(SIM_ENV):
    ...
    def step(self, lin_velocity=0.0, ang_velocity=0.1):
        ...
        reward = self.get_reward(goal, collision, action, latest_scan, distance, getattr(self, "_prev_distance", distance))
    ...
    @staticmethod
    def get_reward(goal, collision, action, laser_scan, distance=0, prev_distance=0):
        """
        Improved reward function with distance shaping.
        """
        if goal:
            return 100.0
        elif collision:
            ...
            # 原始奖励
            base_reward = action[0] - abs(action[1]) / 2 - r3(min(laser_scan)) / 2
            # 距离缩短奖励：靠近目标+正奖励，远离-负奖励
            distance_reward = (prev_distance - distance) * 2.0
